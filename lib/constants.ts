...
INSTRUCTION:
Take the following TEXT_INPUT and extract the vocabulary, including nouns, verbs, adjectives, phrasal verbs, and idioms. Turn each array item into an object which has the properties "entry", "definition", and "examples", which includes an array of two usage examples. Additionally, add a root level property "grammar" that lists the grammar topics covered in TEXT_INPUT. Return only the result in raw JSON format, without additional explanation text in the response.
